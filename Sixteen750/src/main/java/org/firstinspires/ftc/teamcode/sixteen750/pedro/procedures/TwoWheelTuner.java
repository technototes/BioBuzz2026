...
public class TwoWheelTuner extends Procedure {
    ...
    public void run() throws InterruptedException {
        ...
        Inputs.Field<String> forwardPodName = setup
            .s("Forward Encoder Motor Name")
            .withDefault("lf");
        ...
        Inputs.Field<RevHubOrientationOnRobot.LogoFacingDirection> logoDirection = setup
        ...
        Inputs.Field<RevHubOrientationOnRobot.UsbFacingDirection> usbDirection = setup
        ...
        Double forwardTicksPerInchResult = runOpMode(
            new TwoWheelForwardResolution(values, distance.get())
        );
        Double strafeTicksPerInchResult = runOpMode(
            new TwoWheelStrafeResolution(values, distance.get())
        );

        if (
            forwardTicksPerInchResult == null ||
            strafeTicksPerInchResult == null ||
            forwardTicksPerInchResult == 0.0 ||
            strafeTicksPerInchResult == 0.0
        ) {
            abort(
                "Encoder resolution measurement was zero. Complete both pushes before pressing Stop."
            );
            return;
        }
