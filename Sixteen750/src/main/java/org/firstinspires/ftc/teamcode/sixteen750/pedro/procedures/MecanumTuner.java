...
public class MecanumTuner extends Procedure {
    ...
    public void run() throws InterruptedException {
        ...
        Direction frontLeftDirection = testMotor(
            Wheel.FRONT_LEFT,
            "Front Left",
            frontLeftName.get()
        );
        Direction frontRightDirection = testMotor(
            Wheel.FRONT_RIGHT,
            "Front Right",
            frontRightName.get()
        );
        Direction backLeftDirection = testMotor(Wheel.BACK_LEFT, "Back Left", backLeftName.get());
        Direction backRightDirection = testMotor(
            Wheel.BACK_RIGHT,
            "Back Right",
            backRightName.get()
        );
    ...
    private Direction testMotor(Wheel wheel, String displayName, String hardwareName)
        ...
        withDisplay(Display.fourWheelBot(wheel, false), () -> {
            ...
            Inputs inputs = inputs(
                displayName,
                "Determine the " + displayName.toLowerCase() + " motor direction."
            );
            Inputs.Field<Boolean> correctMotorField = inputs
                .b("Did the " + displayName.toLowerCase() + " motor spin?")
                .withDefault(true);
            Inputs.Field<Direction> directionField = inputs.e(
                "Which way did the motor spin?",
                Direction.class
            );
            awaitInputs(inputs);
