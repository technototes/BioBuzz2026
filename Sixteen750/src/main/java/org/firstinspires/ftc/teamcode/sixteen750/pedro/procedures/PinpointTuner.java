...
public class PinpointTuner extends Procedure {
    ...
    public void run() throws InterruptedException {
        ...
        if (podType.get() == PodType.CUSTOM) {
            ...
            customPodScalar = OptionalDouble.of(
                runOpMode(new PinpointCustomPodScalar(distance.get(), pinpointName.get()))
            );
        }

        boolean forwardPodReversed = runOpMode(
            new PinpointForwardDirection(pinpointName.get(), podType.get(), customPodScalar)
        );
        boolean strafePodReversed = runOpMode(
            new PinpointStrafeDirection(pinpointName.get(), podType.get(), customPodScalar)
        );
