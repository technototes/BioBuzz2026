package org.firstinspires.ftc.teamcode.sixteen750.pedro.procedures;
...
public class ForesightTuner extends Procedure {
    ...
    public void run() throws InterruptedException {
        ...
        double forwardVelocity = runOpMode(
            new ForwardVelocity(localizerFunction, drivetrainFunction, distance.get())
        );
        double strafeVelocity = runOpMode(
            new StrafeVelocity(localizerFunction, drivetrainFunction, distance.get())
        );

        Inputs velocityInput = inputs(
            "Velocity",
        ...
        double forwardDeceleration = runOpMode(
            new ForwardDeceleration(localizerFunction, drivetrainFunction, velocity.get())
        );
        double strafeDeceleration = runOpMode(
            new StrafeDeceleration(localizerFunction, drivetrainFunction, velocity.get())
        );

        List<Double> headingBraking = runOpMode(
            new HeadingBraking(localizerFunction, drivetrainFunction)
        );
        ...
        Inputs distanceBrakingInput = inputs(
            "Distance",
        ...
        List<Double> forwardBraking = runOpMode(
            new ForwardBraking(
                localizerFunction,
                drivetrainFunction,
                headingLinear,
        ...
        List<Double> strafeBraking = runOpMode(
            new StrafeBraking(
                localizerFunction,
                drivetrainFunction,
                headingLinear,
        ...
        List<Double> forwardTranslational = runOpMode(
            new ForwardTranslational(localizerFunction, drivetrainFunction)
        );
        List<Double> strafeTranslational = runOpMode(
            new StrafeTranslational(localizerFunction, drivetrainFunction)
        );
            ...
            switch (state) {
                case DRIVE: {
                    if (timer.seconds() > 2) {
                        ...
                        drivetrain.drive(
                            new DrivePowers(0.0, 0.0, -brakingPower * direction),
                            false
                        );
                        state = State.BRAKE;
                        timer.reset();
                ...
                case BRAKE: {
                    if (
                        Math.abs(localizer.velocity().omega) > 0.001 &&
                    ...
                    velocityToBrakingDistance.add(new double[] {
                        measuredVelocity,
                        brakingDistance,
                    });
...
class ForwardBraking extends TuningOpMode<List<Double>> {
    ...
    protected List<Double> runTuningOpMode() throws InterruptedException {
        ...
        while (state != State.DONE && !isStopRequested()) {
            ...
            switch (state) {
                case DRIVE: {
                    if (
                        (direction > 0 && Math.abs(localizer.pose().x()) >= distance) ||
                ...
                case BRAKE: {
                    if (
                        localizer.velocity().toVector2D().magnitude() > 0.25 &&
                        timer.seconds() < MAX_BRAKE_TIME
                    ) {
                        brake(drivetrain, localizer);
                        break;
    ...
    private void drive(Drivetrain drivetrain, Localizer localizer) {
        drivetrain.drive(
            new DrivePowers(power * direction, 0.0, getHeadingPower(localizer)),
            false
        );
    }
...
class StrafeBraking extends TuningOpMode<List<Double>> {
    ...
    protected List<Double> runTuningOpMode() throws InterruptedException {
        ...
        while (state != State.DONE && !isStopRequested()) {
            ...
            switch (state) {
                case DRIVE: {
                    if (
                        (direction > 0 && Math.abs(localizer.pose().y()) > distance) ||
    ...
    private void drive(Drivetrain drivetrain, Localizer localizer) {
        drivetrain.drive(
            new DrivePowers(0.0, power * direction, getHeadingPower(localizer)),
            false
        );
    }
