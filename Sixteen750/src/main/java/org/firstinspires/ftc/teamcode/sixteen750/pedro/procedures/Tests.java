...
public class Tests extends Procedure {
    ...
    public void run() throws InterruptedException {
        ...
        if (algorithm && localizer && drivetrain) followerFunction = hardwareMap ->
            new Follower(
        ...
        switch (selectedTest.get()) {
            case HOLD:
                if (!algorithm) throw new IllegalArgumentException(
                    "Algorithm is required for Hold Test."
                );
            ...
            case INTERPOLATION_CURVED:
                ...
                break;
            case LOCALIZATION:
                if (!drivetrain) throw new IllegalArgumentException(
                    "Drivetrain is required for Localization Test."
                );
                if (!localizer) throw new IllegalArgumentException(
            ...
            case ODOMETRY:
                if (!drivetrain) throw new IllegalArgumentException(
                    "Drivetrain is required for Odometry Test."
                );
                if (!localizer) throw new IllegalArgumentException(
                    "Localizer is required for Odometry Test."
                );
            ...
            case POSE:
                if (!localizer) throw new IllegalArgumentException(
                    "Localizer is required for Pose Test."
                );
                completed = runOpMode(new TestsPose(localizerFunction));
                break;
            case DRIVING:
                if (!drivetrain) throw new IllegalArgumentException(
                    "Drivetrain is required for Driving Test."
                );
...
class TestsCurve extends TuningOpMode<Boolean> {
    ...
    public Boolean runTuningOpMode() throws InterruptedException {
        ...
        Path path1 = curve(
            Pose.zero(),
            new Pose(distance + 0, 0),
            new Pose(distance, distance)
        ).tangent();
        Path path2 = curve(
            new Pose(distance, distance),
            new Pose(distance, 0),
        ...
        while (opModeIsActive()) {
            ...
            switch (test) {
                ...
                case IDLE:
                    ...
                    else if (totalHeading < 0.02) telemetry.addData(
                        "Heading Resolution",
                        "Too high"
                    );
                    else if (totalHeading > 2 * Math.PI) telemetry.addData(
                        "Heading Resolution",
                        "Too low"
                    );
                    else {
