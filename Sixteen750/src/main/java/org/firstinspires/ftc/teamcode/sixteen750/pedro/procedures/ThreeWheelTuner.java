...
public class ThreeWheelTuner extends Procedure {
    ...
    public void run() throws InterruptedException {
        ...
        List<Double> leftOffsets = runOpMode(
            new ThreeWheelOffsets(true, forward, lateral, left.get(1), right.get(1), strafe.get(1))
        );
        if (leftOffsets == null) {
            abort(
        ...
        List<Double> rightOffsets = runOpMode(
            new ThreeWheelOffsets(false, forward, lateral, left.get(1), right.get(1), strafe.get(1))
        );
        ...
        ThreeWheelConfig config = config(
            true,
            forward,
            lateral,
            left.get(1),
...
class ThreeWheelResolution extends TuningOpMode<List<Double>> {
    ...
    protected List<Double> runTuningOpMode() {
        ThreeWheelConfig config = ThreeWheelTuner.config(
            !pod.equals("Right"),
            1.0,
            1.0,
            Encoder.FORWARD,
        ...
        return List.of(
            ...
            movement < 0 ? Encoder.REVERSE : Encoder.FORWARD
        );
    }
}
